...
class BuildingDetailView(NavContextMixin, LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Order by priority (High > Medium > Low) then newest
        from django.db.models import Case, When, IntegerField
        w_qs = w_qs.annotate(
            priority_rank=Case(
                When(priority=getattr(WorkOrder, "PRIORITY_HIGH", "high"), then=3),
                When(priority=getattr(WorkOrder, "PRIORITY_MEDIUM", "medium"), then=2),
                When(priority=getattr(WorkOrder, "PRIORITY_LOW", "low"), then=1),
                default=0,
                output_field=IntegerField(),
            )
...
class UnitCreateView(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not bid:
            return redirect(reverse("buildings_list"))
        self.building = get_object_or_404(Building.objects.only("id","name"), pk=bid)
    ...
    def form_valid(self, form):
        ...
        if getattr(self, "building", None) and not obj.building_id:
